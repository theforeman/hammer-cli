...
describe 'help' do
  class CmdWithHelp < HammerCLI::AbstractCommand
    extend_help do |h|
      h.text _('Details about interface settings')

      h.section 'Available keys for --interface' do |h|
        h.list([
          'mac',
          'ip',
          ['type',         _('One of interface, bmc, bond')],
          'name',
          'subnet_id',
          'domain_id',
          'identifier',
          ['managed',      _('true/false')],
          ['primary',      _('true/false, each managed hosts needs to have one primary interface.')],
          ['provision',    _('true/false')],
          ['virtual',      _('true/false')]
        ])
        ...
        h.section 'For bonds' do |h|
          h.list([
            ['mode',             _('One of balance-rr, active-backup, balance-xor, broadcast, 802.3ad, balance-tlb, balance-alb')],

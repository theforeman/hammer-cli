module HammerCLI
  def self.get_ssl_options
    settings = HammerCLI::Settings
    logger = Logging.logger['SSLoptions']
    ssl_options = {}
    ...
    if ssl_client_cert && ssl_client_key
      ...
      logger.warn("SSL client certificate is set but the key is not, SSL client authentication disabled")
    elsif ssl_client_key
      logger.warn("SSL client key is set but the certificate is not, SSL client authentication disabled")
    end
